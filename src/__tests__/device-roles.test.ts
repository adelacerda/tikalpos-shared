...
describe('Type-level contracts', () => {
  ...
  it('EnrollDeviceResult exposes orgLanguage (relied on by tablet + web for i18n)', () => {
    expectTypeOf<EnrollDeviceResult>().toHaveProperty('orgLanguage').toBeString();
    expectTypeOf<EnrollDeviceResult>().toHaveProperty('role').toEqualTypeOf<DeviceRole>();
  });
});
